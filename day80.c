...
//For example, the shortest path from vertex 1 to vertex 3 is improved via vertex 2 with total cost 8. 
//This process is repeated for all vertices, resulting in the final shortest distance matrix.

#include <stdio.h>
#include <stdlib.h>

#define INF 1e9  // Use a large value that won't overflow during addition
...
int main() {
    ...
    // Read input and convert -1 to INF
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            int val;
            scanf("%d", &val);
